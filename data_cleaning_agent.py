...
def analyze_node(state: CleaningState) -> CleaningState:
    ...
    try:
        ...
        prompt = (
            f"You are a Senior Data Engineer. Analyze this dataset profile and provide a professional status report.\n"
            f"Dataset Statistics:\n{stats_summary}\n\n"
            f"Write a concise 3-5 bullet point summary highlighting data quality issues."
        )
        ...
        report_content = f"⚠️ LLM Analysis Failed: {str(e)}\n\nStats:\n{stats_summary}"
...
def clean_node(state: CleaningState) -> CleaningState:
    try:
        ...
        # -------------------------------------------------
        # STEP 0: Hard target separation (MANDATORY)
        # -------------------------------------------------
        ...
        if target_col and target_col in df.columns:
            ...
            # map common categorical targets
            if y.dtype == "object":
                y = y.str.lower().map({"yes": 1, "no": 0})

            # 🔴 DROP TARGET BEFORE ANY TRANSFORMATION
