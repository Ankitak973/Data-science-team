...
from Backend.agents.utils.eda_helper import EDAProfiler
from Backend.agents.utils.dataset_contract import load_dataset_contract

# Robust custom client to avoid LangChain version issues
# SimpleOllamaClient removed
def get_llm():
    return None
...
def profile_node(state: EDAState) -> EDAState:
    ...
    try:
        ...
        # INJECT CONTRACT INFO
        contract = load_dataset_contract()
        profile['summary']["target_variable"] = contract["target_column"]
        profile['summary']["problem_type"] = contract["problem_type"]
...
def supervised_node(state: EDAState) -> EDAState:
    ...
    try:
        ...
        # No more "if not target check" - we crash if contract is missing (desired)
...
def insight_node(state: EDAState) -> EDAState:
    ...
    try:
        ...
        contract = load_dataset_contract()
        domain_name = contract.get("domain_context", {}).get("domain", "General")
        target_meaning = contract.get("domain_context", {}).get("target_variable_description", "the outcome variable")

        prompt_sys = f"""You are an expert Senior Data Analyst specializing in {domain_name}. 
        Your goal is to provide a comprehensive "Human-in-the-loop" EDA Report with strict focus on DATA INTEGRITY.
        Target variable context: {target_meaning}.
...
- **Terminology**: Use "Relationship" or "Association", NEVER "Causality" or "Driver" unless strictly proven.
- **Statistical Tests**: If you mention significance, you MUST name the test (e.g., Chi-Square, T-Test).
- **Class Balance**: 
    - 45-55% = Balanced
    - 20-45% = Moderately Imbalanced
    - <20% = Highly Imbalanced
- **Visuals**: Recommend specific chart types (e.g., "Boxplot for numeric distribution", "Heatmap for correlation").
"""
        if not summary:
            summary = {"error": "Profiling summary is empty"}
...
def run_eda(cleaned_path: str, target_column: Optional[str] = None, eda_dir: Optional[str] = None, flags: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # LOAD CONTRACT
    contract = load_dataset_contract()
    real_target = contract["target_column"]
