"""
Master Orchestrator
-------------------
Runs the full AVANA Pipeline in sequence:

1. Data Engineer  → Cleaning & preprocessing
2. Data Analyst   → EDA + Visualizations
3. Data Scientist → Modeling + Evaluation + Report

This file contains NO business logic.
It only coordinates existing agents.
"""

from typing import Dict, Any, Optional

# -----------------------------
# DATA ENGINEER
# -----------------------------
from Backend.agents.data_engineer.data_cleaning_agent import clean_file

# -----------------------------
# DATA ANALYST
# -----------------------------
from Backend.agents.data_analyst.data_analyst_orchestrator import (
    DataAnalystOrchestrator
)

# -----------------------------
# DATA SCIENTIST
# -----------------------------
from Backend.agents.data_scientist.orchestrator import (
    run_data_scientist_pipeline
)
from Backend.agents.data_scientist.report_generator import (
    DataScientistReportGenerator
)


class MasterOrchestrator:
    """
    One-click AVANA Consultant
    """

    def run(
        self,
        file_path: str,
        file_type: str = "csv",
        target_column: Optional[str] = None,
        analyst_config: Optional[Dict[str, Any]] = None,
        engineer_config: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        Executes the full pipeline.

        Parameters
        ----------
        file_path : str
            Path to uploaded dataset
        file_type : str
            csv / excel / api / db
        target_column : Optional[str]
            User-selected target (can be None → auto)
        analyst_config : dict
            Options like max_plots, corr_threshold
        engineer_config : dict
            Options like encoding, scaling, schema

        Returns
        -------
        Dict[str, Any]
            Combined output of all agents
        """

        analyst_config = analyst_config or {}
        engineer_config = engineer_config or {}

        # =====================================================
        # 1️⃣ DATA ENGINEER
        # =====================================================
        print("\n🧹 Running Data Engineer Agent...")
        engineer_result = clean_file(
            file_path=file_path,
            file_type=file_type,
            flags={
                **engineer_config,
                "target_column": target_column
            }
        )

        if engineer_result.get("error"):
            return {
                "success": False,
                "stage": "Data Engineer",
                "error": engineer_result["error"]
            }

        cleaned_data_path = engineer_result["output_path"]
        
        # -----------------------------
        # DATASET CONTRACT
        # -----------------------------
        from Backend.agents.utils.dataset_contract import write_dataset_contract
        import pandas as pd
        
        # Load cleaned dataset
        df = pd.read_csv(cleaned_data_path)
        
        # -----------------------------
        # DOMAIN EXPERT (NEW)
        # -----------------------------
        from Backend.agents.utils.domain_expert import DomainExpertAgent
        print("🌍 Running Domain Expert Agent...")
        domain_expert = DomainExpertAgent()
        domain_context = domain_expert.run(df, target_column)
        print(f"📌 Domain identified: {domain_context.get('domain')} (Confidence: {domain_context.get('confidence_score', 'N/A')})")

        # Use normalized target from Engineer if available, otherwise user input, otherwise last col fallback
        final_target = engineer_result.get("normalized_target") or target_column
        
        if not final_target:
             # Fallback: take the last column
             final_target = df.columns[-1]
             print(f"⚠️ No target specified. Defaulting to last column: {final_target}")

        # -----------------------------
        # VALIDATION GATE (NEW)
        # -----------------------------
        from Backend.agents.utils.validator import AnalyticalValidator
        print("🛡️ Running Integrity Validation Gate...")
        
        # Validate Target
        target_check = AnalyticalValidator.validate_target(df, final_target)
        if not target_check["valid"]:
            return {
                "success": False,
                "stage": "Integrity Validator",
                "error": target_check["reason"]
            }
        
        # Validate Domain Alignment
        domain_check = AnalyticalValidator.validate_domain_alignment(final_target, domain_context)
        if not domain_check.get("valid", True):
            return {
                "success": False,
                "stage": "Integrity Validator (Domain)",
                "error": domain_check["reason"]
            }
        
        if domain_check.get("warning"):
            print(f"⚠️ Validation Warning: {domain_check['warning']}")
            
        # Inspect Evidence Integrity
        integrity_status = AnalyticalValidator.inspect_evidence_integrity(df)
        domain_context["evidence_integrity"] = integrity_status

        contract = write_dataset_contract(df, final_target, domain_context=domain_context)
        
        print("📌 Dataset contract locked (with domain context):", contract)

        # USE THE CONTRACT TARGET
        features_target = contract["target_column"]

        # =====================================================
        # 2️⃣ DATA ANALYST
        # =====================================================
        print("\n📊 Running Data Analyst Agent...")
        analyst = DataAnalystOrchestrator(
            cleaned_file_path=cleaned_data_path,
            output_root="data"
        )

        analyst_result = analyst.run_full_pipeline(
            target_column=features_target,
            max_plots=analyst_config.get("max_plots", 30)
        )
        print(f"DEBUG: MasterOrchestrator passing max_plots={analyst_config.get('max_plots', 30)}")

        # =====================================================
        # 3️⃣ DATA SCIENTIST
        # =====================================================
        print("\n🤖 Running Data Scientist Agent...")
        scientist_state = run_data_scientist_pipeline(
            cleaned_data_path=cleaned_data_path,
            user_target=features_target
        )

        # ======================================================
        # FINAL REPORT GENERATION
        # ======================================================
        
        # INJECT DATA ENGINEER METRICS & DOMAIN CONTEXT
        scientist_state["data_engineer_metrics"] = engineer_result.get("metrics", {})
        scientist_state["data_cleaning_report"] = engineer_result.get("report", "")
        scientist_state["domain_context"] = domain_context

        report_generator = DataScientistReportGenerator()
        scientist_report = report_generator.run(scientist_state)

        # =====================================================
        # FINAL RESPONSE
        # =====================================================
        print("\n✅ AVANA Pipeline Completed Successfully")

        return {
            "success": True,

            # Engineer outputs
            "data_engineer": engineer_result,

            # Analyst outputs
            "data_analyst": analyst_result,

            # Scientist outputs
            "data_scientist": {
                "pipeline_state": scientist_state,
                "report": scientist_report
            },

            # Shared context
            "context": {
                "cleaned_data_path": cleaned_data_path
            }
        }
