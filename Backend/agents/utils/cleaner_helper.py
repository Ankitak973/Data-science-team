...
def get_excluded_columns(df: pd.DataFrame, target_column: Optional[str] = None) -> set:
    ...
    Identify columns that should be excluded from encoding/scaling:
      - The target column
      - ID-like columns (ending in "id", named "id", or GUID-like)
    ...
    # GUID / Hash / ID regex
    id_regex = re.compile(r"([a-f0-9]{8,}|[A-Z0-9-]{12,}|^[0-9a-f]{32,})", re.IGNORECASE)

    # Auto-detect ID-like columns
    for col in df.columns:
        col_lower = str(col).lower()
        if col_lower.endswith("id") or col_lower == "id" or col_lower.startswith("id_"):
            exclude_cols.add(col)
            continue

        # Check for sample values looking like hashes or GUIDs
        sample_values = df[col].astype(str).head(10).tolist()
        if any(id_regex.search(val) for val in sample_values):
            exclude_cols.add(col)
            continue
        ...
        if not pd.api.types.is_float_dtype(df[col]):
            try:
                # Minimum 10 rows for this check
                if len(df) > 10 and df[col].nunique() / len(df) > 0.95:
                    exclude_cols.add(col)
            except Exception:
                pass
...
class DataCleaner:
    ...
    def separate_target(self, df: pd.DataFrame, target_column: Optional[str]):
        if target_column and target_column in df.columns:
            ...
            # Flexible Label Encoding for any categorical target
            if y.dtype == "object" or y.dtype.name == "category" or (pd.api.types.is_numeric_dtype(y) and y.nunique() < 10):
                le = LabelEncoder()
                try:
                    # Fill NaNs first to avoid encoder failure
                    y_filled = y.fillna(y.mode()[0] if not y.mode().empty else "Unknown")
                    y = le.fit_transform(y_filled)
                    self.log_action("Target Encoding", f"Target '{target_column}' encoded with classes: {le.classes_}")
                except Exception as e:
                    self.log_action("Target Encoding", f"Warning: Failed to encode target: {e}")
    ...
    def drop_constant_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        dropped = set(initial_cols) - set(df_clean.columns)
        
        if dropped:
            self.log_action("Drop Constant", f"Dropped columns with zero variance: {list(dropped)}")
