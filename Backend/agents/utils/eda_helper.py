...
class EDAProfiler:
    ...
    def supervised_profile(self, df: pd.DataFrame, target: str) -> Dict[str, Any]:
        ...
        if np.issubdtype(y.dtype, np.number):
            ...
            # --- PRECISION ENHANCEMENT: Numeric Statistical Tests ---
            stats_tests = {}
            for col in df.select_dtypes(include=["object", "category", "bool"]).columns:
                 if df[col].nunique() > 1 and df[col].nunique() < 10:
                      ...
                      if len(groups) == 2:
                           ...
                           stats_tests[f"{col}_ttest"] = {"p_value": float(p_val), "significant": p_val < 0.05}
                      elif len(groups) > 2:
                           f_stat, p_val = f_oneway(*groups)
                           stats_tests[f"{col}_anova"] = {"p_value": float(p_val), "significant": p_val < 0.05}
            info["statistical_tests"] = stats_tests
        else:
            counts = y.value_counts().to_dict()
            info["class_distribution"] = counts
            info["imbalance_ratio"] = float(max(counts.values()) / min(counts.values()))
            
            # --- PRECISION ENHANCEMENT: Categorical Statistical Tests ---
            stats_tests = {}
            for col in df.columns:
                ...
                if df[col].nunique() > 1 and df[col].nunique() < 50:
                    ...
                    if contingency.size > 0:
                        ...
                        stats_tests[f"{col}_chi2"] = {"p_value": float(p), "significant": p < 0.05}
            info["statistical_tests"] = stats_tests
