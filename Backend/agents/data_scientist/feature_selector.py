...
class FeatureSelectorAgent:
    ...
    def run(self, test_size: float = 0.2, random_state: int = 42) -> Dict:
        ...
        # 🔐 ACCURACY BOOST: One-Hot Encode categorical behavioral drivers (Contract, Payment, etc)
        X = pd.get_dummies(X, drop_first=True)
        X.columns = [str(c) for c in X.columns] # Defensive: Ensure string column names
